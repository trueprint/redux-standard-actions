...
describe('makeActionCreators', () => {
  it('should throw an error when given arguments that contain a non-string', () => {
    ...
    expect(() => makeActionCreators({ ACTION_1: undefined }, [])).to.throw(TypeError, expectedError)
    expect(() => makeActionCreators('ACTION_1', true)).to.throw(TypeError, expectedError)
  })

  it('should throw an error when given bad payload creators', () => {
    expect(
      () => makeActionCreators({ ACTION_1: [] })
  ...
  it('should create actions from an object and action types', () => {
    const { action1, action2, action3, action4 } = makeActionCreators({
      ACTION_1(key, value) {
        return { [key]: value }
      },
